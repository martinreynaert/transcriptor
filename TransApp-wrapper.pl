...
if ($collection =~ /GEO/){
$pid = open(README, " perl $ROOTDIR/TransApp.pl -b /exp/sloot/usr/local/bin/ -c $INPUTDIR -d $OUTPUTDIR -e $ROOTDIR/JRCnames/JRCnames.LexicalSenses.3cols.UTF8.15.OnlyWithSenseIDs.NamesOnlyOnePerLine.sortu.tsv.cut1.lc.UNDERSCORE.chars -f $ROOTDIR/JRCnames/JRCnames.LexicalSenses.3cols.UTF8.15.OnlyWithSenseIDs.NamesOnlyOnePerLine.sortu.tsv.cut1.clip500.ld2.charconfus -i $ROOTDIR/TransApp/GeoNames/cities1000.2.txt -j $ROOTDIR/TransApp/GeoNames/alternateNames.txt -k $PROJECT -l 4 -m 5 -n 100000000000 -o $collection -p 10 -q en -r $interlang -s $targetlang -t $INPUTDIR/input.txt >$OUTPUTDIR/output.txt |")  or die "Couldn't fork: $!\n";
}
