...
print STDERR
"TICCL_OPTS: a: $opt_a b: $opt_b c: $opt_c d: $opt_d e: $opt_e f: $opt_f g: $opt_g h: $opt_h i: $opt_i j: $opt_j k: $opt_k l: $opt_l m: $opt_m n: $opt_n o: $opt_o p: $opt_p q: $opt_q r: $opt_r s: $opt_s t: $opt_t\n";
...
$tocorpusfoci =
  ...
  . $prefix
  . '.toanahash.tsv'
  ; ##Was bij onze testen file met anagramwaarden voor de fociwoorden. Moet 1 string zijn die nog moet omgezet worden door TICCL-anahash!! De anagramwaarde moet naar een bestandje waarnaar $corpusfoci verwijst

open( FOCI, ">$tocorpusfoci" )
  || die "Could not at all open $tocorpusfoci: $!\n";
...
##الشيخ@ar@occ1024696@6
	...
	while ( $list1 = <K> ) {
		...
		if ( @LIST1[0] !~ /^\s*$/ ) {
			$JRCnames{ @LIST1[0] }{ @LIST1[2] }{ @LIST1[1] } += @LIST1[3]
			  ; ##Interessanter om de namen te accumuleren voor een bepaalde taal, maar dan niet zoekbaar op naam! Tweede hash maken daarvoor?
			$langseen{ @LIST1[1] }++;
			@LISTB = split '_', @LIST1[0];
			foreach $listitem1 (@LISTB) {
				if ( $listitem1 !~ /^\s*$/ ) {
					$JRCnames{$listitem1}{ @LIST1[2] }{ @LIST1[1] } += @LIST1[3]
					  ; ##Houdt niet alle items bij, voor eoa reden alleen voornaam? Haakje stond verkeerd!!!
				}
			}
			@LISTB = ();
		}
...
##Abu_Bakar_Ba@sense1998@nl
	...
	while ( $list = <L> ) {
		...
		if ( !defined( $langseen{ @LIST[2] } ) ) {
			$langseen{ @LIST1[2] }++;
			if ( @LIST[0] !~ /^\s*$/ ) {
				$JRCnames{ @LIST[0] }{ @LIST[1] }{ @LIST[2] } += 1
				  ; ##Interessanter om de namen te accumuleren voor een bepaalde taal, maar dan niet zoekbaar op naam! Tweede hash maken daarvoor?
				@LIST2 = split '_', @LIST[0];
				foreach $listitem (@LIST2) {
					if ( $listitem !~ /^\s*$/ ) {
						$JRCnames{$listitem}{ @LIST[1] }{ @LIST[2] } += 1
						  ; ##Houdt niet alle items bij, voor eoa reden alleen voornaam? Haakje stond verkeerd!!!
					}
				}
				@LIST2 = ();
...
##Dit klopt niet. URL- en POST-gevallen moeten niet verder meegenomen worden! : Opgelost met ##to else langseen 117
				if ( ( $type =~ /1/ ) or ( $type =~ /0/ ) )
				{ ##Verzamel hier alleen de canonieke vormen. Beter niet doen, maar wel bij weergeven voorkeur geven aan deze? 120: Nu ook die zonder type-aanduiding!
					if ( @LIST[3] !~ /^\s*$/ ) {
						@LIST[3] =~ s/ /_/g;
						if ( $type =~ /1/ ) {
							if ( $GEOfrq{ @LIST[1] } > 1 ) {
								$JRCnames{ @LIST[3] }{ @LIST[1] }{ @LIST[2] } =
								  $GEOfrq{ @LIST[1] };
							}
							else {
								$JRCnames{ @LIST[3] }{ @LIST[1] }{ @LIST[2] } =
							...
							$JRCnames{ @LIST[3] }{ @LIST[1] }{ @LIST[2] } += 1
							  ; ##Interessanter om de namen te accumuleren voor een bepaalde taal, maar dan niet zoekbaar op naam! Tweede hash maken daarvoor?
						}
...
print STDERR
"RUN_TICCL-anahash: ALPH: $alph BACKGROUND: $tsv ARTIFRQ: $artifrq TOCORPUSFOCI: $tocorpusfoci\n";
...
if ( ( $check != 0 ) and ( $check != $anaPunct ) ) {
	print STDERR
"RUN_TICCL-indexerNT: input: $anahash > $charconfus > $corpusfoci > $confuslist\n";
`${TOOLDIR}TICCL-indexerNT -t $threads --hash $anahash --charconf $charconfus --foci $corpusfoci -o $confuslist`;

	print STDERR
"RUN_TICCL-LDcalc: $out > $confuslist.indexNT > $anahash > $frqlist > $alph > $LD > $threads > $artifrq > $out.ldcalc\n";
...
##Hier werden alleen voorkomens van een naamvariant als CC geteld. Wellicht beter de CCs te ranken volgens corpusfrequentie!!
	...
	foreach $nameseen ( sort { $seenticcl{$b} <=> $seenticcl{$a} }
		keys %seenticcl )
	{
...
##Nu in hash %JRCnames de ID ophalen
	...
	print STDERR
"WILLNOTRUN_TICCL-indexerNT: input: $anahash > $charconfus > $corpusfoci > $confuslist CHECK: $check\n";
}
...
if (    ( $name =~ /^$/ )
	and ( ( $interlang =~ /ru/ ) or ( $targetlang =~ /ru/ ) ) )
{    ##Name = leeg?????
...
##Do automatic transcription only
	...
	print STDOUT
"<FONT COLOR=\"\#910a22\">Your Query:</FONT>\n\n\t\t\t<h>$originput<\h>\n";
	print STDOUT
...
foreach $JRCname ( sort keys %JRCnames ) {
	if ( $JRCname =~ /$name/ ) { ##MOET KUNNEN DIENEN OM DE SEARCH TE NARROWEN!!
		...
		foreach $ID ( keys %{ $JRCnames{$JRCname} } ) {
			...
			#foreach $lang (keys %{ $JRCnames{$JRCname}{$ID} }) {
			foreach $lang ( sort { $b <=> $a }
				keys %{ $JRCnames{$JRCname}{$ID} } )
			{
				...
				print STDERR
"JRCNAMEIDLANGLANGFRQ: $JRCname ID: $ID L: $lang LF: $langfrq ALL: >$JRCnames{$JRCname}{$ID}{$lang}<\n";
				$NameID{$ID} += $langfrq;
		...
		foreach $JRCname ( sort keys %JRCnames ) {
		 ...
		 #print STDERR "HASH: $JRCname, $ID, $lang: $JRCnames{$JRCname}{$ID}\n";
					if ( $NameID =~ /^$ID$/ ) {
						if ( $interlang =~ /$lang/ ) {
							print STDERR
"HASHINTER: $JRCname, $ID, $lang: $JRCnames{$JRCname}{$ID}{$lang}\n";
						...
						if ( $targetlang =~ /$lang/ ) {
							print STDERR
"HASHTARGET: $JRCname, $ID, $lang: $JRCnames{$JRCname}{$ID}{$lang}\n";
...
#foreach $frq  (sort { $NamesInter{$interName}{$b} <=> $NamesInter{$interName}{$a} } keys %{ $NamesInter{$interName} }){
			foreach $frq ( sort { $b <=> $a }
				keys %{ $NamesInter{$interName} } )
			{
				print STDERR
				  "IN2: $interName FRQ: $frq $NamesInter{$interName}{$frq}\n";
				if ( $originput =~ / / ) {    ##Hoe sorteren op bigram of niet??
					if ( $NamesInter{$interName}{$frq} =~ /[-_]/ ) {
						...
						if ( $tel < 4 ) {
							...
							print STDERR
"$interlangUC: $interName >> $frq >> $NamesInter{$interName}{$frq}\n";
							print STDOUT
"\t\t$NamesInter{$interName}{$frq}&emsp;<FONT COLOR=\"\#269ABC\">Identifier:</FONT> $interName\t<FONT COLOR=\"\#269ABC\">Frequency:</FONT> $frq\n\n";
							if ( $interlang =~ /ru/ ) {
...
"\n<FONT COLOR=\"\#910a22\">Corpus versions target language:</FONT> $targetlang\n\n";
			...
			#print STDOUT "\tName variant:\tIdentifier:\tFrequency:\n\n";
			$tel2 = ();
			foreach $frq2 ( sort { $b <=> $a }
				keys %{ $NamesTarget{$targetName} } )
			{
				print STDERR
"TN2: $targetName FRQ2: $frq2 $NamesTarget{$targetName}{$frq2}\n";
				if ( $originput =~ / / ) {    ##Hoe sorteren op bigram of niet??
					if ( $NamesTarget{$targetName}{$frq2} =~ /[-_]/ ) {
						...
						if ( $tel2 < 4 ) {
							...
							print STDERR
"$targetlangUC: $targetName >> $frq2 >> $NamesTarget{$targetName}{$frq2}\n";
							print STDOUT
"\t\t$NamesTarget{$targetName}{$frq2}&emsp;<FONT COLOR=\"\#269ABC\">Identifier:</FONT> $targetName\t<FONT COLOR=\"\#269ABC\">Frequency:</FONT> $frq2\n\n";
							if ( $mode =~ /GEO/ ) {
								print STDOUT
"<FONT COLOR=\"\#269ABC\">URL Wikipedia:</FONT>\n\n\t\t\t$URL{$targetName}\n";
							}
...
#print STDOUT "\t\t<FONT COLOR=\"\#269ABC\">_________________________________________________</FONT>\n";
	...
	if ( $teller == 4 ) {
		...
		print STDOUT
"\t\t<FONT COLOR=\"\#910a22\">_________________________________________________</FONT>\n";
	}
}    #to foreach NameID
